...
// Configure storage based on environment or preference
const USE_JSON_FILE = process.env.RUNTIME === 'desktop' || true; // Default to JSON file storage for desktop app

// Create appropriate storage instance
const storageInstance = USE_JSON_FILE 
  ? new JsonFileStorage() 
  : new PostgresStorage();

console.log(`💾 Using ${USE_JSON_FILE ? 'JSON File' : 'PostgreSQL'} storage for data persistence`);

// Initialize admin user on startup (JsonFileStorage handles this internally)
if (!USE_JSON_FILE && 'initializeAdminUser' in storageInstance) {
  (storageInstance as any).initializeAdminUser().catch((error) => {
